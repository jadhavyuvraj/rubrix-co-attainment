...
- **27 pytest tests passed.** The suite covers inclusive and fractional boundaries, zero/empty/all/none attainment, invalid thresholds, CRUD, duplicates, cross-course validation, atomic batch rejection, cascades, seed idempotency, UTC timestamps, packaged static frontend serving, PostgreSQL driver selection, and persistent SQLite startup.
