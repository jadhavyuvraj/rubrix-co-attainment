...
export function LoadingSkeleton() {
  return (
    <div className="space-y-5 animate-pulse" role="status" aria-label="Loading course data">
      <div className="h-24 rounded-2xl bg-slate-200/60" />
      <div className="h-80 rounded-2xl bg-slate-200/60" />
      <span className="sr-only">Loading course data</span>
    </div>
  )
}
...
export function Toast({ notice, onClose }: { notice: Notice | null; onClose: () => void }) {
  ...
  return (
    <div className={`toast ${notice.type}`} role={notice.type === 'error' ? 'alert' : 'status'}>
      {notice.type === 'success' ? <CheckCircle2 size={20} /> : <CircleAlert size={20} />}
      <span>{notice.message}</span>
      <button onClick={onClose} className="icon-button" aria-label="Dismiss notification">
        <X size={17} />
      </button>
    </div>
  )
