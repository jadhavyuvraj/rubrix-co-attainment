...
export const api = {
  ...
  attainment: (id: number, threshold: number) =>
    request<Attainment[]>(`/courses/${id}/attainment?threshold=${threshold}`),
  save: <T>(path: string, value: unknown, method = 'POST') =>
    request<T>(path, { method, body: JSON.stringify(value) }),
